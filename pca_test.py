...
df_raw = pd.read_csv("folk_version_02/combined_wide.csv")

cols = ["candidate_id", "name", "party_code"] + [f"q{i}_answer_value" for i in range(1, 26)]
df = df_raw[cols].copy()
df.to_csv("folk_version_02/pca_input.csv", index=False)
...
for i, party in enumerate(party_df.index):
    ...
    ax.annotate(party, (coords[i, 0], coords[i, 1]), fontsize=12,
...
ax.set_title("PCA of party positions (mean responses)")
...
plt.tight_layout()
plt.savefig("folk_version_02/pca_parties.png", dpi=150)
plt.show()
